...
Per tal de definir un objecte de [**SageMath**]{style="color: blue"} que
...
h(x,y)=x$^\wedge$`<!-- -->`{=html}2-x\*y+ln(x$^\wedge$`<!-- -->`{=html}2+y$^\wedge$`<!-- -->`{=html}2)
...
expr= ln(3\*x$^\wedge$`<!-- -->`{=html}3+sqrt(x-1))
...
g=(x$^\wedge$`<!-- -->`{=html}2-y$^\wedge$`<!-- -->`{=html}2+sin(x\*y)).function(x,y)

show(g)

show(g(2,pi))

g=(x$^\wedge$`<!-- -->`{=html}2-y$^\wedge$`<!-- -->`{=html}2+sin(x\*y)).function(y,x)
...
fyz=y$^\wedge$`<!-- -->`{=html}2-3\*y\*z+2\*z$^\wedge$`<!-- -->`{=html}2

f(y,z)=y$^\wedge$`<!-- -->`{=html}2-3\*y\*z+2\*z$^\wedge$`<!-- -->`{=html}2
...
el fons, és el que utilitza [**SageMath**]{style="color: blue"} per a
...
[**SageMath**]{style="color: blue"} ho fa automàticament normalment,
...
(les interfícies de SAGE solen indentar automàticament tot el que va
darrera del `if \bullet :` i, per tant, no cal indentar explícitament el
...
respon \"2 es menor que 3\"  ja que la primera instrucció dóna el valor
de 2 a la variable `a`. Si canviem la primera instrucció per `a=5`,
respondrà \"5 no es menor que 3\". Fixeu-vos que he utilitzat la funció
`str` per convertir el valor de la variable `a`, que és un nombre, a una
cadena (string en anglés), que després he ajuntat (estrictament
s'anomena concatenar cadenes, que vol dir ajuntar-les posant-les una
seguida de l'altre, i es fa amb el `+`).
...
sum(a$^\wedge$`<!-- -->`{=html}2 for a in range(101))
:::

**Nota2:** Sovint quan treballem amb
[**SageMath**]{style="color: blue"}és millor utilitzar la funció
`srange` que la funció `range`: la diferència és que la primera et dóna
enters de [**SageMath**]{style="color: blue"}i la segoja enters de
Python; i amb els enters de Python no hi ha definits certs mètodes per
...
sum(a$^\wedge$`<!-- -->`{=html}2 for a in range(101) if is_prime(a))
:::

no funciona, però si si posem ``

::: list

sum(a$^\wedge$`<!-- -->`{=html}2 for a in srange(101) if is_prime(a))
:::

Observeu que quan assigneu un valor enter a una variable, el
[**SageMath**]{style="color: blue"}el considera un enter de Sage i no de
Python.
...
mxi=20  #Nombre maxim de repeticions del proces
...
[**SageMath**]{style="color: blue"} `previous_prime` està molt més
...
`524`. La funció de [**SageMath**]{style="color: blue"} corresponent és
`next_prime`.
...
`True` or `False` (els booleans) depenen de si el nombre que li passem
...
volem que retorni la funció, en cada cas. Un cop exectuta un return la
...
o un error si no ho fos; això es pot fer usant la funció `type`, que et
respon el \"tipus\" que és el valor passat: en el cas que ens interessa,
...
primers menors que 100 i amb reste 1 al dividir per 3: ``

::: list
:::

Les funcions, en principi, poden no necessitar cap dada incial ni tant
sols retornar res. Per exemple ``
...
Collatz (o també Conjectura del $3\,x+1$) que afirma:
...
> sempre en la seqüència $4,\ 2,\ 1,\ 4,\ 2,\ 1,\ \ldots$*
...
seqüències que apareixen s'ha estudiat des de nombrosos punts de vista.
Un petit programa que permet observar les seqüències que apareixen quan
...
3.  Suposeu que, sigui on sigui, tenim 100 portes tancades i numerades.
    ...
    [**SageMath**]{style="color: blue"} llistant al final quines portes
...
4.  (*Passeig aleatori*) Suposeu que ens trobem a la posició $(0,0)$ del
    ...
    [**SageMath**]{style="color: blue"} fins que torneu a l'origen o
