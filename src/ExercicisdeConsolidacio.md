...
1. Feu els gràfics necessaris per tal de posar de manifest que l’equació $f(x) = 0$ té tres solucions reals.
2. Calculeu amb find_root() les aproximacions numèriques d’aquestes tres solucions i poseu-les en una llista ordenada de nom Sol. 
3. Feu un gràfic de la funció f entre $-1$ i $1$ en el que surti en blau la part on $f(x) > 0$, en vermell la part on $f(x) < 0$ i surti una marca de color verd en els punts on $f(x) = 0$.
