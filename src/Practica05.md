---
jupyter:
  title: 'Pràctica 5: Polinomis, vectors, matrius i espais vectorials'
  authors:
  - name: Marc Masdeu
  - name: Xavier Xarles
  jupytext:
...
# Polinomis, vectors, matrius i espais vectorials
...
Ara bé, tal com ho hem fet, la variable que nosaltres veiem al imprimir un polinomi \(en el nostre cas, la `x`\), no la podem utilitzar per a escriure el polinomi. 

```sage
type(x)
```

El que podem fer és donar\-li el nom `x` al polinomi corresponent. O millor, podem declarar que la variable, que és R.gen\(\), li podrem dir x. 
