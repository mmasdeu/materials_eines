...
Feu una classe Z2, que només té dos objecte, `0` i `1`. Per crear un element es pot admetre que se li passi un enter `n` i el nombre és `n%2`. La classe ha de tenir redefinits els mètodes màgics `__repr__`, `__add__` i  `__mul__`. 
...
També podeu fer un `__repr__` per tal que us mostri un triangle com la llista dels seus punts, o qualsevol versió que us agradi més. 
